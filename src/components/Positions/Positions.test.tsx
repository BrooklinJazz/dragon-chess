...
import { A2Pawn, mockMove, A7Pawn, B2Pawn } from "../../constants/pieces";
...
test("disabled positions _ no moving piece", () => {
  const { getByTestId } = customRender(<Positions />);
...
test("moving A2Pawn _ enabled A3 & A4 position", () => {
  ...
  const store = configureMockStore({ ...initialGameState, ...mockState });
...
test("initiateMove and movePiece _ A2Pawn _ fresh board", () => {
  const store = configureMockStore({ ...initialGameState, pieces: [A2Pawn] });
